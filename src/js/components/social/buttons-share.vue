<template>
    <div class="block-share">
        <button
            ...
            type="button">
            <img v-bind:src="getButtonConfig(button).icon"/>
        </button>
    </div>
</template>
  ...
  module.exports = {
    ...
    methods: {
      ...
      openPopup: function (button) {
        let buttonData = this.getButtonConfig(button);
        ...
        request.onload = function() {
          if (request.status >= 200 && request.status < 400) {
            ...
            console.log('success!');
            var data = 
            window.open(self.getButtonConfig(button).getPopupUrl(self.getUrl(), self.title, request.responseText, self.text),'','toolbar=0,status=0,width=626,height=436');
          } else {
